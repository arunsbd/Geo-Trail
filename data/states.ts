...
export const STATE_BY_CODE = new Map(
  STATES.map((state) => [state.code, state] as const),
);

export const STATE_CODE_BY_NAME = new Map(
  STATES.map((state) => [state.name.toLowerCase(), state.code] as const),
);
...
export function findState(input: string): StateInfo | null {
  const normalized = input.trim().replace(/\s+/g, " ");
  const possibleCode = normalized.toUpperCase();

  if (isStateCode(possibleCode)) {
    return STATE_BY_CODE.get(possibleCode) ?? null;
  }
